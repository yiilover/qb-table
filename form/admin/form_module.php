...
if($job=="list")
{
	if(!table_field("{$_pre}module",'use_yzimg')){
		$db->query("ALTER TABLE  `{$_pre}module` ADD  `use_yzimg` TINYINT( 1 ) NOT NULL ,ADD  `template` VARCHAR( 255 ) NOT NULL ,ADD  `email` VARCHAR( 255 ) NOT NULL ;");
	}
...
elseif($job=="set")
{
	...
	$usetitle[intval($rsdb[usetitle])]=' checked ';
	$repeatpost[intval($rsdb[repeatpost])]=' checked ';
	$use_yzimg[intval($rsdb[use_yzimg])]=' checked ';
...
elseif($action=="set")
{
	...
	$db->query("UPDATE {$_pre}module SET allowpost='$allowpost',allowview='$allowview',endtime='$postdb[endtime]',name='$postdb[name]',about='$postdb[about]',usetitle='$postdb[usetitle]',repeatpost='$postdb[repeatpost]',statename='$postdb[statename]',use_yzimg='$postdb[use_yzimg]',email='$postdb[email]' WHERE id='$id'");
	jump("修改成功","$FROMURL",1);
}
...
function make_show_table($rs){	
	...
	$show="<tr> <td class='tdL'>{$rs[title]}:</td> <td class='tdR'><table width='100%' border='0' cellspacing='0' cellpadding='0' style='TABLE-LAYOUT: fixed;WORD-WRAP: break-word;'><tr><td>{\$rsdb[{$rs[field_name]}]}&nbsp;{$rs[form_units]}&nbsp;&nbsp;$show</td></tr></table></td></tr>";	
	if($rs[form_type]=='pingfen'){	
