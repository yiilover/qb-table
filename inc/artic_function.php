...
function isspesort($fid,$flag=0){
	global $webdb,$pre,$db;
	$rs=$db->get_one("SELECT fup FROM {$pre}sort WHERE fid='$fid'");	
	if($flag==1){
		return $rs[fup];
	}
...
function get_sub_cat($fid){
	...
	foreach($listdb as $row){
		$pic = !empty($row['logo'])?$webdb[www_url]."/".$webdb['updir']."/".$row['logo']:$webdb[www_url]."/images/default/nopic.jpg";
		$desc = mb_strlen($row[descrip])>100?mb_substr($row[descrip],0,100)."...":mb_substr($row[descrip],0,100);
		$url = $webdb[www_url]."/list.php?fid=".$row['fid'];
		$data[] = array(
		  'fid' => $row['fid'],
		  'name' => $row['name'],
		  'url' => $url,
...
function get_module_content($table,$fid,$mid,$page=1, $num=50){
	...
	$query=$db->get_one("SELECT config FROM {$pre}article_module WHERE id=".$mid);
	$config = unserialize($query[config]);
	foreach($config[field_db] as $v){
		if($v[IfListShow]==1) continue;
		$fdb[]=$v[field_name];
	}
	...
	//if($k>80) continue;
		...
		foreach($r as $i=>$v){
			...
			if(in_array($i, array('id','aid','rid','fid','uid'))) continue;
			if(in_array($i, $fdb)) continue;
			$data[$k][module][$i]=$v;
...
function get_module_content2($table,$aid,$mid){
	global $db,$pre,$webdb;
	$query=$db->get_one("SELECT * FROM {$pre}".$table." WHERE aid=".$aid);	
		
	foreach($query as $k=>$r){		
		if(in_array($k, array('id','aid','rid','fid','uid'))) continue;
		$data[$k]=$r;				
	}
	return $data;	
}
