...
/*产品小分类的专用的分页方法*/
function getpage2($table,$choose,$url,$rows=20,$total='',$fid){
	global $page,$db;
	if(!$page){
		$page=1;
	}
	...
	if(!$total && $table){
		$query=$db->get_one("SELECT COUNT(*) AS num  FROM $table $choose");
		$total=$query['num'];
		$query=$db->query("SELECT fid FROM $table $choose");
		$k=1;
		while($rs = $db->fetch_array($query)){
			$fids[$k]=$rs[fid];
			$totalpage2=$rs[fid];
	...
	foreach($fids as $kk=>$v){
		if($v==$fid) $m=$kk;
	}
	$nextpage2 = $fids[$m+1];
	$uppage2 = $fids[$m-1];	
	if($nextpage>$totalpage || empty($nextpage2)){
		$nextpage=$totalpage;
		$nextpage2 = $totalpage2;
	}
	if($uppage<1 || empty($uppage2)){
		$uppage=1;
		$uppage2=$fids[1];
	}	
	...
	for($j=$s;$j<=$b;$j++){
		if($fids[$j]==$fid){
			...
			$show.=" <a href=\"$url$fids[$j]\" title=\"第{$j}页\">$j</a>";
		}
	}
	$showpage="<a href=\"$url$fids[1]\" title=\"首页\">首页</A> <a href=\"$url$uppage2\" title=\"上一页\">上一页</A>  {$show}  <a href=\"$url$nextpage2\" title=\"下一页\">下一页</A> <a href=\"$url$totalpage2\" title=\"尾页\">尾页</A> <a href='#'><font color=red>$page</font>/$totalpage/$total</a>";
    if($totalpage>1){
		return $showpage;
	}
}
